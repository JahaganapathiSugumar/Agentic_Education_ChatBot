...
def process_message(sender, text):
    ...
    if text.lower().strip() in ['cancel', 'stop', 'menu', 'start', 'exit']:
        ...
        options = ["Ask an OS Question", "Create a Worksheet", "Create a PPT", "Upload Material", "Podcast from Image", "Summary from Image", "Create a Video"]
        send_menu_message(sender, menu_text, options)
    ...
    if not current_state:
        ...
        if lower_text == "ask an os question" or lower_text == "ask os question":
        ...
        if lower_text == "create a worksheet" or lower_text == "create worksheet":
    ...
    if current_state == "awaiting_worksheet_type":
        try:
            ...
            if worksheet_content_result and worksheet_content_result.get("source") == "generated_worksheet_text":
                ...
                send_interactive_message(sender, "I've sent the PDFs. How should I post this to Classroom?", ["As Announcement", "As Assignment", "Don't Post"])
            else:
    ...
    if current_state == "awaiting_classroom_post_type":
        if text.lower() == "as an announcement":
            user_states[sender] = "awaiting_announcement_description_choice"
            send_interactive_message(sender, "What should be the description for the announcement?", ["Auto-generate", "Type Manually"])
        elif text.lower() == "as an assignment":
            user_states[sender] = "awaiting_assignment_title"
    ...
    if current_state == "awaiting_assignment_title":
        ...
        user_states[sender] = "awaiting_assignment_deadline"
        send_whatsapp_message(sender, "Got it. When is this due? (e.g., 'tomorrow', 'next Friday')")
        return

    if current_state == "awaiting_assignment_deadline":
        try:
            due_date = parse_deadline(text)
            title = user_temp_data[sender]['assignment_title']
            drive_file_id = user_temp_data[sender].get('drive_file_id') # Will be None for forms

            result = post_assignment(CLASSROOM_COURSE_ID, title, drive_file_id, due_date)
            create_calendar_event(title, due_date)
            end_conversation_and_show_menu(sender, result['result'])
        except (ValueError, KeyError) as e:
            print(f"Error in assignment deadline state: {e}")
            end_conversation_and_show_menu(sender, "An error occurred. Please start over.")
        return
